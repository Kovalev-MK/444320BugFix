...
using avis.PowerOfAttorneyModule.RecallPowerOfAttorneyAssignment;

namespace avis.PowerOfAttorneyModule.Client
{
  partial class RecallPowerOfAttorneyAssignmentActions
  {
    ...
    public virtual void RecallPowerOfAttorneys(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      if (dialog.Show() == DialogButtons.Ok)
      {
        foreach(var document in documents)
        {
          ...
          {
            e.AddWarning(string.Format("Доверенность с номером '{0}' была отозвана ранее.", powerOfAttorney.RegistrationNumber ?? "незарегистрированная доверенность"));
            continue;
          }
    ...
    public virtual bool CanRecallPowerOfAttorneys(Sungero.Domain.Client.CanExecuteActionArgs e)
    {
      return true;
    }
