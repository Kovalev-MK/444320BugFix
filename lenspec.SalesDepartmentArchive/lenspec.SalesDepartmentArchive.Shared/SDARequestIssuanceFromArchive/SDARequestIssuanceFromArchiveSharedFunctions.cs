...
{
  partial class SDARequestIssuanceFromArchiveFunctions
  {
    ...
    public override void FillName()
    {
      ...
      /* Имя в формате:
        <Вид документа> №<№ заявки> от <Подразделение> от <Дата создания>.
       */
      using (TenantInfo.Culture.SwitchTo())
      {
        if (!string.IsNullOrWhiteSpace(_obj.RequestNumber))
          name += Sungero.Docflow.OfficialDocuments.Resources.Number + _obj.RequestNumber;
