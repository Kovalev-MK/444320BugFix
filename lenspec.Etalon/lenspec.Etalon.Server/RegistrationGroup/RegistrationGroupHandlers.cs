...
namespace lenspec.Etalon
{
  partial class RegistrationGroupServerHandlers
  {
    ...
    public override void BeforeSave(Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      var managers = Roles.GetAll().FirstOrDefault(r => r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationManagersRole);
      var responsiblesIds = RegistrationGroups.GetAll(gr => !Equals(gr, _obj) && gr.Status == Sungero.CoreEntities.DatabookEntry.Status.Active)
      ...
      if (needChangeRoles)
      {
        ...
        if (_obj.Status == Sungero.CoreEntities.DatabookEntry.Status.Active)
        {
          ...
          var nameRole = Roles.GetAll(r => (r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.ClerksRole && needIncludeInClerkRole) ||
                                      (r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationContractualDocument && _obj.CanRegisterContractual == true));
          includedRoles.AddRange(nameRole);
          if (_obj.CanRegisterIncoming == true)
          {
            var registrationIncomingRole = Roles.GetAll(r => r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationIncomingDocument).FirstOrDefault();
            if (registrationIncomingRole != null)
              includedRoles.Add(registrationIncomingRole);
          }
          if (_obj.CanRegisterOutgoing == true)
          {
            var registrationOutgoingRole = Roles.GetAll(r => r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationOutgoingDocument).FirstOrDefault();
            if (registrationOutgoingRole != null)
              includedRoles.Add(registrationOutgoingRole);
          }
          if (_obj.CanRegisterInternal == true)
          {
            var registrationInternalRole = Roles.GetAll(r => r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationInternalDocument).FirstOrDefault();
        ...
        var excludedRoles = Roles.GetAll(r => r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationIncomingDocument || 
                                         r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationOutgoingDocument ||
                                         r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationInternalDocument || 
                                         r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.ContractsResponsible ||
                                         r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.ClerksRole || 
                                         r.Sid == Sungero.Docflow.PublicConstants.Module.RoleGuid.RegistrationContractualDocument)
