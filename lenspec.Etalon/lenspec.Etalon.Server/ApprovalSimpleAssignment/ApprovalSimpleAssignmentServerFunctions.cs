...
namespace lenspec.Etalon.Server
{
  partial class ApprovalSimpleAssignmentFunctions
  {
    ...
    /// <summary>
    /// Построить инструкцию по заданию.
    /// </summary>
    /// <returns>Инструкция по заданию.</returns>
    [Remote(IsPure = true)]
    public StateView GetInstruction()
    {
      ...
      {
        var instructionText = CourierShipments.CourierShipmentsApplications.Resources.Instruction ?? string.Empty;
    ...
    {
      var task = Sungero.Docflow.ApprovalTasks.As(_obj.Task);
      var stageLiteModel = Functions.ApprovalTask.GetStage(task, lenspec.Etalon.ApprovalStage.StageType.SimpleAgr);
