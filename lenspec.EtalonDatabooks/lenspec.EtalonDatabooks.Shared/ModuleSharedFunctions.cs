...
namespace lenspec.EtalonDatabooks.Shared
{
  public class ModuleFunctions
  {
    ...
    /// <summary>
    /// Проверить, откуда была создана версия документа.
    /// </summary>
    [Public]
    public bool IsNullOrImported(Sungero.Content.IDocumentHistory historyRow)
    {
      return historyRow == null || historyRow.OperationDetailed == Sungero.Content.DocumentHistory.OperationDetailed.Import;
    }
    
    /// <summary>
    /// Получить названия НОР.
    /// </summary>
    /// <param name="businessUnits">Список НОР.</param>
    /// <returns>Отображаемые названия НОР.</returns>
    [Public]
    public virtual string GetBusinessUnitsNames(List<Sungero.Company.IBusinessUnit> businessUnits)
    {
      return string.Join("; ", businessUnits.Select(x => x.Name));
    }
