...
{
  partial class ProjectBuildingPermitDocumentCollectionAccountingObjectAccountingObjectPropertyFilteringServerHandler<T>
  {

    public virtual IQueryable<T> CollectionAccountingObjectAccountingObjectFiltering(IQueryable<T> query, Sungero.Domain.PropertyFilteringEventArgs e)
    {
  ...
  partial class ProjectBuildingPermitDocumentCollectionObjectAnProjectObjectAnProjectPropertyFilteringServerHandler<T>
  {
  ...
  partial class ProjectBuildingPermitDocumentOurCFPropertyFilteringServerHandler<T>
  {
  ...
  // Добавлено avis.
  partial class ProjectBuildingPermitDocumentServerHandlers
  {
    /// <summary>
    /// До сохранения.
    /// </summary>
    ...
    {
      // Проверяем на дубли.
      var projectBuildingPermitDoc = lenspec.EtalonDatabooks.ProjectBuildingPermitDocuments.GetAll(p => p.Id != _obj.Id && p.NumberRNS == _obj.NumberRNS && p.DateRNS == _obj.DateRNS);
      if (projectBuildingPermitDoc.Count() > 0)
      {
