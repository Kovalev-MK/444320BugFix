...
namespace lenspec.LocalRegulations
{
  partial class AcquaintanceReportClientHandlers
  {
    public override void BeforeExecute(Sungero.Reporting.Client.BeforeExecuteEventArgs e)
    {
      ...
      var department = dialog.AddSelect(Reports.Resources.AcquaintanceReport.Department, false, Sungero.Company.Departments.Null);
      ...
      if (this.AcquaintanceReport.Document != null && this.AcquaintanceReport.Document.HasVersions)
      {
        ...
        var versionsWithTasks = new List<Sungero.Content.IElectronicDocumentVersions>();
        
        var tasks = Sungero.Docflow.PublicFunctions.OfficialDocument.Remote.GetAcquaintanceTasks(this.AcquaintanceReport.Document);
        ...
        {
          var dialogVersions = versionsWithTasks.Select(v => string.Format("{0} {1}", Sungero.Docflow.Resources.Version, v.Number.ToString()))
            .ToArray();
          dialogVersion = dialog.AddSelect(Sungero.Docflow.Resources.Version, true, dialogVersions.LastOrDefault()).From(dialogVersions);
        }
      }
      ...
      if (dialog.Show() == DialogButtons.Ok)
      {
        if (dialogVersion != null && dialogVersion.Value != null)
          AcquaintanceReport.DocumentVersion = dialogVersion.Value.Replace(string.Format("{0} ", Sungero.Docflow.Resources.Version), string.Empty);
