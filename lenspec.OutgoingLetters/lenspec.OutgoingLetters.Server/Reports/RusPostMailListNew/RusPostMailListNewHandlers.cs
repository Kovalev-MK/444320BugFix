...
{
  partial class RusPostMailListNewServerHandlers
  {
    ...
    {
      // Фильтруем по дате.
      var result = avis.PostalItems.PostalItems.GetAll(p => p.DateCreated != null && p.DateCreated.Value >= RusPostMailListNew.DateStart.Value && p.DateCreated.Value <= RusPostMailListNew.DateEnd.Value);
      ...
      if (RusPostMailListNew.Vid == "Ценные письма")
